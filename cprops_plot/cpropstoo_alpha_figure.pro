PRO cpropstoo_alpha_figure, intable = intable $
                       , inner_pc = inner_pc $
                       , ring_pc = ring_pc $
                       , outer_pc = outer_pc $
					   , inprop = inprop $
...
resolve_ind = where( array_re gt 0.5, res_ct)
 ...
 ;    d_vrms=d_vrms[res_inds]
	sub_xtitle = 'log $\alpha$!Dobs,vir!N' ;'log $\alpha$!Dobs,vir!N'
endif
 ;  endif else begin
 ...
 ;  endelse
  ...
  ; Mlum = (props.MASS_EXTRAP)[resolve_ind] ; alpha_co used in cprops run was 5.0, but then changed (with the latest pymakeplots) and can potentially change in the future too
  ...
  ind = where(finite(logMlum) and finite(logMvir) and finite(d_logMlum) $
    ...
    ; print, ct ; cloud peaknum = 1394 has d_logMvir=NaN due to d_logR = NaN 
  logMlum = logMlum[ind]
  logMvir = logMvir[ind]
  ...
  ; Simple statistics method. Not preferred in this script
...
endelse
  ...
  print, slope_A[0], ' check if it is using default values without fitting, like zero'
  print,'============================================================='
...
; Plot Mlum vs. Mvir
  ...
  device, filename=outfile, bits_per_pixel=8, /encapsulated, $
		xsize=7., ysize=7., /inches, /color, xoffset=0, yoffset=0, SET_FONT='Times', /TT_FONT
  ...
  ploterror, 6.8, 5.8, md_logMlum, md_logMvir, psym=3, errthick=4, $ 
    ytitle=ytitle, xtitle='log(!8M!X!Dgas!N / M'+sunsymbol()+'!N)', charsize = 2.0, xthick=5, ythick=5, charthick=5, $
      xrange=[3.9,7.1], yrange=[3.9,7.1],xst=1,yst=1, position = [0.14,0.15, 0.98, 0.98], $ ; [minval, maxval]
    hatlength=0.0
  ...
  cgtext, 6.52,6.6,'!N$\alpha$!Dobs,vir!N = 1',  ORIENTATION=45, charsiz=1.3
  cgtext, 6.22,6.6,'!N$\alpha$!Dobs,vir!N = 2',  ORIENTATION=45, charsiz=1.3
  ...
  plothist, log_alpha, bin = binsize, axiscolor = 'black', $
	  color = cgcolor('black'), $
       boxplot = 0, thick = 4, $
	   xtitle=sub_xtitle, ytitle = 'Number of GMCs', $
	   charsize=1.5, xthick =4.0, ythick=4.0, charthick=8.0, $
       position = [0.72,0.27,0.96,0.5], $
       xtickinterval=tickinterval, xminor=floor(tickinterval/0.1), $
	   xticklen=0.08, yticklen=0.06, $
	   xrange=[-1.3,1.3], yrange=[0.0, 200] ; hist_xrange max(yhist)+70
