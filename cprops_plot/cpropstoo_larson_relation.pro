PRO cpropstoo_larson_relation , inprop = inprop $
					   , inner_pc = inner_pc $
					   , ring_pc = ring_pc $
             , outer_pc = outer_pc $
					   , xrange = xrange $
...
; READ GMC TABLE 
  ...
  if keyword_set(nofitting) then begin ; store the fitting result (20250513) of all clouds
  slope_A0 = [0, 0.28627320, 0]
  zp_A0 = [0, 0.22549395, 0]

  slope_A1 = [0, 2.4084793, 0]
  zp_A1 = [0, 1.6084267,  0]

  slope_A2 = [0, 2.3010096, 0]
  zp_A2 = [0, 3.4464609, 0]
endif
  ...
  ; outer_ind, ring_ind, inner_ind, all_ind ; to fit different regions, change this line manually
...
; FIT LOG(vrms) VS LOG(R) WITH ERROR
  ...
  ; Testing r_correlate p-value
  ; seed = !NULL
  ; arrayx = RANDOMU(seed, 3000)
  ; arrayy = RANDOMU(seed, 3000)
  ; print, 'testing', r_correlate(arrayx, arrayy)
...
if ~keyword_set(nofitting) then begin
  ...
  slope_A1 = [0, median(slope1), mad(slope1)]
  zp_A1 = [0, median(zp1), mad(zp1)]

endif

  corr1 = (r_correlate(logR[pick_ind], logLum[pick_ind]))
  print, '******************************************************'
  ...
  print, 'ZP OF LOG(R) VS. LOG(LUM):', zp_A1[1], '+/-', $
  	  zp_A1[2]
  print, 'SPEARMAN RANK CORRELATION COEFFICIENT:', corr1
  print, '******************************************************'
...
; FIT LOG(vrms) VS LOG(LUM) WITH ERROR
  ...
  slope_A2 = [0, median(slope2), mad(slope2)]
  zp_A2 = [0, median(zp2), mad(zp2)]
endif

  corr2 = (r_correlate(logvrms[pick_ind], logLum[pick_ind]))
  print, '******************************************************'
...
!P.FONT = 1
  ...
  ; y_MW=-0.3+0.82*x_MW
  ; ; oplot, x_MW, y_MW, linestyle=1, thick=6.; original
  ; cgoplot, x_MW, y_MW, linestyle=1, thick=5., color='MAGENTA' ; , using thick=6.
  ...
  cglegend,title=['NGC1387','MW disc','MW CMZ'], $ ; ,'NGC4429' ;psym=9, symsize=1.5, length=0.0,
       color=['black','black','Orange'], location=[0.14, 0.9], $ ; ,'MAGENTA'
       linestyle=[0,1,1], thick=5.,$ ; ,1
