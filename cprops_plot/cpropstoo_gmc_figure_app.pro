PRO cpropstoo_gmc_figure_app, infile=infile, inmask=inmask, $
...
; PLOT IMAGE MOM0
  ...
  device, filename=outfile, bits_per_pixel=8, /encapsulated, $
	  xsize=8, ysize=8, /inch, /color, xoffset=0, yoffset=0, /TT_FONT, SET_FONT='Times' ; SET_FONT='Times',
  ...
  if keyword_set(log) then begin
    ...
    CGIMAGE,alog10(im),pos=pos,stretch=1, /noerase,/Keep_aspect_ratio, $
	  minvalue=minvalue, maxvalue=maxvalue;, AXKeywords={XTickFormat:'(A1)'} ;YTICKFORMAT="(A1)", XTICKFORMAT="(A1)"
  endif else begin
	nan_ind = where(im eq 0.0, nan_ct)
	if nan_ct gt 0.0 then $
		im[nan_ind] = !values.f_nan
    ...
    CGIMAGE,im,pos=pos,stretch=1, /noerase,/Keep_aspect_ratio, $
	  ...
	  bottom = 0.1;, XTICKFORMAT="(A1)", YTICKFORMAT="(A1)"
  endelse
  cgloadct,0,/SILENT
  ...
  cgAxis, /xAxis, xrange=[xcrange[0]*arctopc,xcrange[1]*arctopc], $
     xtitle=' ', font=1.95, charsize=2.2, xthick=10, charthick=5,xst=1,yst=1;,xtickformat=ytickformat
  cgAxis, /yAxis, yrange=[ycrange[0]*arctopc,ycrange[1]*arctopc], $
     ytitle=' ', font=1.95, charsize=2.2, ythick=10, charthick=5,xst=1,yst=1;,ytickformat=ytickformat ; charsize used to be 2.2

  ; changed by Eric
  cgtext, 0.5, 0.03, "RA - RA!Dcentre!N (arcsec)", alignment=0.5, charsize = 3, charthick=5.0, orientation = 0,/normal
  cgtext, 0.08, 0.5, 'Dec. - Dec.!Dcentre!N (arcsec)', alignment=0.5, charsize = 3, charthick=5.0, orientation = 90,/normal
  cgtext, 0.5, 0.94, 'RA - RA!Dcentre!N (pc)', alignment=0.5, charsize = 3, charthick=5.0, orientation = 0,/normal
  cgtext, 0.96, 0.50, 'Dec. - Dec.!Dcentre!N (pc)', alignment=0.5, charsize = 3, charthick=5.0, orientation = -90,/normal
  ...
  ;   and (props.vrms_extrap_deconv ge dv/2.)
  resolves = props.resolve_spatial * props.resolve_spectral
  ...
  ; tvellipse, 0.015, 0.010, 0.47, 0.95, 0.0, $ ; was 0.012, 0.006, 0.54, 0.88, 0.0
  ;       noclip=0, color=cgcolor('blue'), thick=3.0, /normal
  ; xyouts, 0.5, 0.94, $ ; was 0.56, 0.87, Eric
  ;     strtrim(res_ct_cleaned,2)+' resolved GMCs', color=cgcolor('blue'), $ ; used to be resolve_ct
	;   charsize=2.7, charthick = 4.0, /normal

  ; tvellipse, 0.015, 0.010, 0.47, 0.90, 0.0, $ ; was 0.012, 0.006, 0.54, 0.85, 0.0
  ;       noclip=0, color=cgcolor('GRN5'), thick=3.0, /normal
  ; xyouts, 0.5, 0.89, $ ; was 0.56, 0.84 Eric
