...
;
;   20170213  LJ  add keyword do_bootstrap so only calculate variables
...
;    ... ADD FIELDS FOR PROPERTIES
     if keyword_set(properties) then begin
        ...
        props = create_struct(props, "radell_deconv_uc", nan)  

        props = create_struct(props, "virmass_radell_deconv", nan)
        props = create_struct(props, "virmass_radell_deconv_uc", nan)
...
; -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
     ...
     props.virmass_radell_deconv = $
        ...
        props.vrms_gauss_extrap_deconv^2   ;(LJ)
