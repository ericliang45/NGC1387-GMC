...
;   20171229  LJ  set keyword 'diskmode' and add input parameter 'gal_par'
...
; Establish the distance metric to those kernels.
			if diskmode ne 1 then begin
              darr = sqrt((xpos[k]-x_kernel[possible])^2+$
                        (ypos[k]-y_kernel[possible])^2+$
                        vwt*(vpos[k]-v_kernel[possible])^2)
            endif else begin
              darr = calc_distance(x1=xpos[k], $
                       y1=ypos[k], x2=x_kernel[possible], $
                       y2=y_kernel[possible], in_pa=pa, in_inc=inc)
            endelse
