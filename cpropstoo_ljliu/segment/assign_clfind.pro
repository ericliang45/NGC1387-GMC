...
; HISTORY
;
;   20161213  LJ  modify from 'spacing=2*sigma' to 'spacing=0.2*sigma'
;   20170628  LJ  add keyword nonuniform and input rmsfile
;   20171214  LJ  modify from 'spacing=0.2*sigma' to 'spacing=0.1*sigma'
;   20171226  LJ  set keyword 'diskmode' and add input parameter 'gal_par'
;                 gal_par = [inc[degree], pa[degree]] of galaxy
;
...
; &%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%
  ...
  if keyword_set(diskmode) then begin
	  if n_elements(gal_par) ne 2 then begin
	      ...
	      inc = gal_par[0]    ; in [degree]
	      pa = gal_par[1]     ; in [degree]
      endelse
  endif
...
;             FIGURE OUT THE DISTANCE BETWEEN EACH PIXEL AND THIS KERNEL
			  if not keyword_set(diskmode) then begin 
                dist = (shared_x -minikern_x[kern_ind[k]])^2 + $
                       (shared_y -minikern_y[kern_ind[k]])^2 + $
                       (shared_v -minikern_v[kern_ind[k]])^2
			  endif else begin
                dist = calc_distance(x1=minikern_x[kern_ind[k]], $
                       y1=minikern_y[kern_ind[k]], x2=shared_x, $
					   y2=shared_y, in_pa=pa, in_inc=inc)
			  endelse
